        ...
        Movies.map((movies, index) => (
          ...
          >
            <Link to={`/Movies/${movies.id}`} className="flex flex-col h-full">
              <img
                className={`h-[20rem] w-full rounded-lg object-cover transition-transform duration-300 transform hover:scale-105 ${isImageLoaded ? '' : 'border-2 border-gray-300'}`}
                key={index}
                src={`https://image.tmdb.org/t/p/original${movies.poster_path}`}
                onLoad={() => setIsImageLoaded(true)}
              />
              <div className="flex flex-col justify-between flex-grow mt-4">
                <h3 className="text-white text-lg font-semibold">{movies.title}</h3>
                <div className="mt-2">
